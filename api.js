...
export const fetchGifArrayData = async (keyWord, dataArray) => {
    const giphyResponse = await fetch(`${API_URL}search?q=${createKeyWordURL(keyWord)}&${API_URL_END}&limit=20`).then(response => response.json());
...
export const fetchSingleGifData = async (ID) => {
    const giphyResponse = await fetch(`${API_URL}${ID}?${API_URL_END}`).then(response => response.json());
